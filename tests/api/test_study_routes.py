...
# --- recall probe, schedule, and the chat arm -----------------------------------


def test_recall_options_are_opaque_and_not_part_of_the_case_payload(client):
    from secondlook.study.views import recall_truth_ids

    case = load_case_set(DEFAULT_CASE_SET_DIR / "pilot_v0.yaml").cases[0]
    payload = client.get(f"/api/study/cases/{case.case_id}?arm=diff_first").text
    assert "recall" not in payload  # served separately, at the recall step

    options = client.get(f"/api/study/cases/{case.case_id}/recall").json()["options"]
    ids = {o["id"] for o in options}
    assert recall_truth_ids(case) <= ids
    assert len(options) == len(case.update_events) + len(case.recall_distractors)
    assert all(len(i) == 8 and int(i, 16) >= 0 for i in ids)  # opaque hex, no "u1"/"d1" hints


def test_schedule_covers_all_three_arms_and_names_the_participant(client):
    body = client.get("/api/study/schedule/0").json()
    assert body["participant_id"] == "P01"
    assert [i["arm"] for i in body["items"]] == ["diff_first", "dashboard", "chat"]
    assert client.get("/api/study/schedule/1").json()["participant_id"] == "P02"


def test_schedule_rejects_a_pool_too_small_for_the_request(client):
    assert client.get("/api/study/schedule/0?cases_per_arm=3").status_code == 422


def _first_case():
    return load_case_set(DEFAULT_CASE_SET_DIR / "pilot_v0.yaml").cases[0]


def test_chat_session_opens_with_the_same_findings_the_other_arms_show(client):
    case = _first_case()
    session = client.post("/api/study/chat/sessions", json={"case_id": case.case_id}).json()
    briefing = session["history"][0]
    assert briefing["role"] == "assistant"
    for finding in case.system_output.findings:
        assert finding.claim in briefing["content"]
    assert briefing["sources_count"] == len(case.system_output.findings)
    assert [s["citation_index"] for s in briefing["sources"]] == [1, 2, 3]


def test_chat_never_receives_or_reveals_the_answer_key(client):
    case = _first_case()
    session = client.post("/api/study/chat/sessions", json={"case_id": case.case_id}).json()
    reply = client.post(
        f"/api/study/chat/sessions/{session['id']}/turns", json={"message": "What changed?"}
    ).json()
    text = str(session) + str(reply)
    for issue in case.seeded_issues:
        assert issue.description not in text
    assert "reported_supersessions" not in text  # the diff is not injected into chat


def test_chat_turn_uses_case_findings_as_sources_not_the_graph(client, monkeypatch):
    def boom(*args, **kwargs):
        raise AssertionError("study chat must not query the knowledge graph")

    monkeypatch.setattr("secondlook.chat.engine.retrieve_evidence_for_turn", boom)
    case = _first_case()
    session = client.post("/api/study/chat/sessions", json={"case_id": case.case_id}).json()
    reply = client.post(
        f"/api/study/chat/sessions/{session['id']}/turns", json={"message": "Summarise."}
    ).json()
    assert reply["turn"]["sources_count"] == len(case.system_output.findings)
...
def test_the_citation_backstop_applies_to_study_chat(client, monkeypatch):
    """#124's overclaim check runs against the case's findings, so a model that
    cites a finding number that does not exist is withheld -- and one that cites a
    real finding number is not."""
    case = _first_case()
    session = client.post("/api/study/chat/sessions", json={"case_id": case.case_id}).json()
    url = f"/api/study/chat/sessions/{session['id']}/turns"

    monkeypatch.setattr("secondlook.chat.engine.build_client", lambda _m: _Fake("See finding [9]."))
    bad = client.post(url, json={"message": "why?"}).json()
    assert "withheld" in bad["assistant_message"]["content"]

    monkeypatch.setattr("secondlook.chat.engine.build_client", lambda _m: _Fake("See finding [2]."))
    ok = client.post(url, json={"message": "why?"}).json()
    assert ok["assistant_message"]["content"] == "See finding [2]."


def test_ordinary_chat_sessions_cannot_use_the_study_turn_route(client):
    from secondlook.chat.session import create_session

    plain = create_session()
    response = client.post(f"/api/study/chat/sessions/{plain.id}/turns", json={"message": "hello"})
    assert response.status_code == 404


def test_chat_session_for_an_unknown_case_is_404(client):
    assert client.post("/api/study/chat/sessions", json={"case_id": "nope"}).status_code == 404
